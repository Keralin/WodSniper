...
def get_classes_by_day(day_of_week):
    ...
    try:
        ...
        if not classes:
            ...
            current_app.logger.info(f'Day {day_of_week}: Found {len(unique_times)} unique time slots for {target_date.strftime("%Y-%m-%d")}')
            if len(unique_times) <= 4:
                ...
                current_app.logger.info(f'Special day detected ({len(unique_times)} slots). Searching for typical schedule...')
